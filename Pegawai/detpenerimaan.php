  ...
  <div class="panel-body">
                <table class="table table-striped table-bordered table-list">
                 ...
                 // $hasil = mysqli_query($koneksi,"select br.kode_bar,br.nama_bar, tr.id_trima, hp.jumlah_his, hp.harga_his, hp.subtotal from history_penerimaan hp join penerimaan tr on hp.id_trima=tr.id_trima join barang br on hp.kode_bar=br.kode_bar where tr.id_trima='$id'  ") ; 
                   if ($hasil === FALSE) {
                        ...
                        <td><?php
                              ...
                              if($sts=='0'){
                                ...
                                <input type="hidden" name="idps" value='.$data[0].'><input type="hidden" class="btn btn-info btn-xs" name="bar" value='.$data[1].'>
                                <button type="submit" style="background-color:rgb(255,0,0);" class="btn btn-info btn-xs">Hapus</button> <a href=editpenerimaan.php?id='.$data[0].' class="btn btn-info btn-xs" type="button">edit</a>
                                </form>'."";
                              }if($sts=='1'){
                                echo "Tersimpan! ";
                                }
                  ...
                  <td colspan="2">Rp.<?php echo number_format(@$tot); ?>,-</td>
                  </tr>
                </table>
              </div>
                        </body>
